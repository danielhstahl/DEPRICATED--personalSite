<html>
    ...
    <div class="Container">
        <div class='col-md-8'>
            ...
            In a continuous setting, the "first hitting time" of a process is defined as the first time a process is equal to a level <span lang="latex">m</span>.  If the process is random, the first hitting time is also random.  With few exceptions, the density of the first hitting time of even a single dimensional Stochastic Differential Equation (SDE) is not explicitly known.  However, finding the distribution numerically is not computationally difficult.  
            <br>
            ...
            The characteristic function of a random variable <span lang="latex">\tau</span> can be numerically inverted to recover the density.  Recalling that the characteristic function is defined as 
            <span class='eq' lang="latex"> \phi(u)=\mathbb{E}\left[e^{ui\tau}\right]</span> and letting <span lang="latex"> g(X_t)=e^{uit}h(X_t)</span>, then by Dynkan's formula we have 
            <span class='eq' lang="latex"> \phi(u)=\mathbb{E}[e^{ui\tau}]=h(X_0)+\int_0 ^ \tau \left(\mathcal{A}h+uih\right) dt</span>
            
            With <span lang="latex"> h(X_\tau)=h(m)=1 </span>.  <span lang="latex"> \phi(u) </span> is then the solution to the ODE 
            
            <span class='eq' lang="latex"> \mathcal{A}h+uih=0</span> 

            
            With terminal condition <span lang="latex">h(m)=1 </span>.  This ODE can be efficiently solved numerically (eg, by using finite difference methods) and then numerically inverted to obtain the density function of <span lang="latex">\tau</span>.  The following "r" functions provide examples of this inversion.  
