    ...
    function CreditRisk(options){
        ...
        function computeV(u){ //returns array of complex
            ...
            for(var i=0; i<n;i++){
                ...
                vp=math.add(vp, math.multiply(helperV, tmpPhi)); //w_{j, k}p_j*(e^{i*u*l_j}-1)                             vp=math.add(vp, -sig*sig*u*u*a*b*a*b); //diffusion part...scales with expected exposure                          vp=math.add(vp, math.complex(0, u*a*b*alph)); //drift part...scales with expected exposure...but this seems to cause issues          
            }
            v.push(vp);
            for(var j=1; j<m; j++){ 
                var vp=math.complex(0, 0);
